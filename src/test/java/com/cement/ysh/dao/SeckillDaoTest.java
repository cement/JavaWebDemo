...
//告知junit spring配置文件
@ContextConfiguration({"classpath:spring/spring-db-dao.xml"})
public class SeckillDaoTest {
    ...
    public void selectAll() {
        ...
        params.add(1);
        params.add(2);
        List<Map> seckills = seckillDao.selectAll(params);
        for (int i = 0; i < seckills.size(); i++) {
            Map map = seckills.get(i);
    ...
    public void selectByMap() {
        ...
        params.put("offset",2);
        params.put("count",4);
        List<Map> seckills = seckillDao.selectByMap(params);
        System.out.println("************************************************************************");
        for (int i = 0; i < seckills.size(); i++) {
            Map map = seckills.get(i);
